...
var QueryGenerator = {
  ...
  attributeToSQL: function(attribute, options) {
    ...
    {
      const { query: rawQuery, stored } = attribute.generated;
      const query = (rawQuery.startsWith('(') && rawQuery.endsWith(')')) ? rawQuery : `(${rawQuery})`;
      const type = stored ? 'STORED' : 'VIRTUAL';
      template += ` GENERATED ALWAYS AS ${query} ${type}`;
    }
